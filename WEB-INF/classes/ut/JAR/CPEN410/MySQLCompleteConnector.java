...
/******
	This class manage a connection to the Mini Facebook database and it should not be accessed from the front End. 
*/
public class MySQLCompleteConnector{
	...
	private String DB_URL="jdbc:mysql://localhost/Facebook";
	...
	*/
	...
	/***********
		doInsertion method
			This method performs an insertion to the database in a specific column
			@parameters:
				values: values to be inserted 
				table: table to be updated
				filter: specific column to be inserted 
			@returns:
				boolean value: true: the insertion was ok, an error was generated
	*/
	
	public boolean doInsert(String table,String filter, String values)
	{
		boolean res=false;
		String charString ="INSERT INTO "+ table + filter + " values (" + values +");";
		System.out.println(charString);
		//try to insert a record to the selected table
		try{
			 res=stmt.execute(charString);
	...
	/***********
		doUpdate method
			This method performs an update to the database in a specific column
			@parameters:
				values: values to be inserted 
				table: table to be updated
				whereClause: specific column to be updated
			@returns:
				boolean value: true: the update was ok, an error was generated
	*/
	
	public boolean doUpdate(String values, String tables, String where){
		boolean res = false;
		String charString = "UPDATE " + tables + " SET " + values + "Where " + where + " ;";
		System.out.println(charString);
		//try to insert a record to the selected table
		try{
			res = stmt.execute(charString);
